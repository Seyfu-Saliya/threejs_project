...
function createJebena() {
    ...
    // Traditional clay material with blackened finish (very rustic & matte)
...
function createChocho() {
    ...
    const bodyPoints = [
        ...
        new THREE.Vector2(0.12, 0.8),
        new THREE.Vector2(0.1, 1.0),
        new THREE.Vector2(0.13, 1.1)
    ];
    const geo = new THREE.LatheGeometry(bodyPoints, 32);
    
    const canvas = document.createElement('canvas');
    canvas.width = 512;
    canvas.height = 512;
    const ctx = canvas.getContext('2d');
    ctx.fillStyle = '#4a2c17'; // Darker clay/leather
    ctx.fillRect(0, 0, 512, 512);
    
    // Add "leather strap" textures
